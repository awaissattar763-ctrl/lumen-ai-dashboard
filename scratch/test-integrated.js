...
async function test() {
  try {
    console.log('Requiring pdf.worker...');
    // Explicitly load the worker so it registers inside pdfjs-dist
    require('pdfjs-dist/legacy/build/pdf.worker.mjs');
    
    // Check if pdfjs is set on globalThis
    console.log('globalThis.pdfjs exists?', !!globalThis.pdfjs);
